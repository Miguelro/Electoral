...
#'
#'\deqn{pp=\sum_{i}p_{i}(x_{i}-\overline{x}_{p})^{2}}{pp=sum(pi*(xi-mean(x_p)))}
#'for \eqn{i =1,  2, \ldots,n } y pi los porcentajes ( en tanto por uno) de votos o escaños
#'del partido i
#'
#'
#'@param votos Es un vector de números enteros, conteniendo los votos o escaños de todas las candidaturas.
#'@param escala Es un vector de número decimales entre 1 y 10  para indicar la ubicación ideológica de cada partido
#'de tal manera que 1 significa totalmente a la izquierda y 10 la ideología es totalmente a
#'la derecha.
#'
#'@return Devuelve el valor obtenido por el indicador
#'
#'@examples
#'
...
#'extremos de la posición de la escala. La fórmula de calculo es la siguiente:
#'
#'\deqn{Pd=\sqrt{\sum p_{i}\left[\frac{\overline{x}_{i}-\overline{x}_{p}}{4.5}\right]}}{Pd=sqrt(sum(pi*(xi-xp)^2))}
#'for \eqn{i =1,  2, \ldots,n } y pi los porcentajes  de votos o escaños
#'del partido i, xi la posición idelógica del partido i  y xp el promedio ideológico
#'ponderado del sistema de partidos en la escala 1-10
#'
#'@param votosPorcentaje Es un vector de números decimales, conteniendo \bold{los porcentajes ( en tantos por ciento ) de votos o escaños} de todas las candidaturas.
#'@param escala Es un vector de número decimales entre 1 y 10  para indicar la ubicación ideológica de cada partido
#'de tal manera que 1 significa totalmente a la izquierda y 10 la ideología es totalmente a
#'la derecha.
#'
...
#'@examples
#'
#'a <- data.frame(prov=c("Alava","Albacete","Valladolid"),PP=c(20,30,10),PSOE=c(34,12,45),
#'stringsAsFactors = F)
#'
#'INSP(a)
#'
#'
#'b<-data.frame(prov=c("1","2"),A=c(400,190),B=c(200,1000),stringsAsFactors = F)
#'INSP(b)
#'
#'@export
INSP <- function(datos){
